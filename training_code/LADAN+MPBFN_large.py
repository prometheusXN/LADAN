...
with tf.Graph().as_default():
    ...
    with tf.name_scope('MPBFM'):
        feature = keras.layers.Dense(config_2.hid_size)(fact_repr)

        law_embedding_layer = Embedding(config_2.num_law, config_2.hid_size, trainable=True, mask_zero=False)

        accu_embedding_layer = Embedding(config_2.num_accu, config_2.hid_size, trainable=True, mask_zero=False)

        term_embedding_layer = Embedding(config_2.num_term, config_2.hid_size, trainable=True, mask_zero=False)

        law_preds1 = Dense(config_2.num_law, activation='softmax')(feature)  # res_law

        law_input_2 = tf.placeholder(shape=(batch_size, config_2.num_law,), dtype=tf.int32)

        law_embedded_sequences = law_embedding_layer(law_input_2)
        law_merge = dot([law_preds1, law_embedded_sequences], axes=(1, 1))  # (130 * 256)
        # law_merge = law_preds1 @ law_embedded_sequences
        law_merge_semantic = Dense(config_2.hid_size, activation='elu', use_bias=False)(law_merge)  # sem
        ...
        accu_embedded_sequences = accu_embedding_layer(accu_input_2)
        accu_merge = dot([accu_preds1, accu_embedded_sequences], axes=(1, 1))
        # accu_merge = accu_preds1 @ accu_embedded_sequences
        accu_merge_semantic = Dense(config_2.hid_size, activation='elu', use_bias=False)(accu_merge)  #sem
    ...
    # term_input = Input(shape=(config_2.num_term,), dtype='int32')
    term_embedded_sequences = term_embedding_layer(term_input_2)
