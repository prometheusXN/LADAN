...
from law_processed.law_processed import get_law_graph_large
import keras.layers
...
with tf.Graph().as_default():
    ...
    law_input, graph_list_1, graph_membership, neigh_index = get_law_graph_large(law_relation_threshold, 'data/w2id_thulac.pkl', 15, 100)
    ...
    decoder = LSTMDecoder(config, 118, 130)
    ...
    ############################--------------initialized over---------------########################
    f_train = pk.load(open('legal_basis_data_big/train_processed_thulac_Legal_basis_large.pkl', 'rb'))
    f_test = pk.load(open('legal_basis_data_big/test_processed_thulac_Legal_basis_large.pkl', 'rb'))
